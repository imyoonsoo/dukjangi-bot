...
├── conventions/
└── public/
    └── assets/                   # 이미지와 폰트 (정적 import + URL 직접 참조)
```

## 폴더별 역할

- `app/`: 페이지, 레이아웃, 라우트만. `page.tsx`는 얇게 유지하고 로직은 훅이나 도메인 모듈에서 가져와 조립
- `features/<domain>/`: 한 도메인의 로직. 여러 화면과 라우트가 공유하는 훅, 상태, 저장, 타입, 상수. 예: `features/chat/`
